...
function EventCard({ event }: { event: Evento }) {
  const accent = event.categoria === 'Cultural' ? 'from-violet-500 to-indigo-500' : event.categoria === 'Académico' ? 'from-blue-600 to-cyan-400' : 'from-emerald-500 to-teal-400'
  return (
    <article className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/70">
      <div className={`relative h-44 bg-gradient-to-br ${accent} p-5`}>
        ...
        <span className="relative inline-flex rounded-full border border-white/30 bg-white/15 px-3 py-1 text-xs font-semibold text-white backdrop-blur">{event.categoria}</span>
      ...
      <div className="flex flex-col gap-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-lg font-bold leading-tight text-slate-900 group-hover:text-blue-700">{event.titulo}</h3>
        ...
        <p className="line-clamp-2 text-sm leading-6 text-slate-500">{event.descripcion}</p>
        <div className="flex flex-col gap-2 border-t border-slate-100 pt-3 text-sm text-slate-600">
          <span className="flex items-center gap-2"><CalendarDays className="size-4 text-blue-600" aria-hidden="true" />{formatDate(event.fecha_evento)}</span>
          <span className="flex items-center gap-2"><MapPin className="size-4 text-blue-600" aria-hidden="true" />{event.es_virtual ? 'Evento virtual' : event.lugar}</span>
        </div>
      </div>
    </article>
  )
...
export default function Page() {
  const [events, setEvents] = useState<Evento[]>(fallbackEvents)
  ...
  useEffect(() => {
    ...
    async function loadEvents() {
      ...
      const { data, error } = await supabase.from('eventos').select('*').eq('estado', 'aprobado').order('fecha_evento', { ascending: true })
  ...
  const filteredEvents = useMemo(() => events.filter((event) => {
    const matchesQuery = `${event.titulo} ${event.descripcion ?? ''}`.toLowerCase().includes(query.toLowerCase())
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section id="eventos" className="mx-auto max-w-7xl px-5 pb-20 lg:px-8">
        ...
        {loading ? <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{[1,2,3].map((i) => <div key={i} className="h-[430px] animate-pulse rounded-2xl bg-slate-200" />)}</div> : filteredEvents.length ? <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{filteredEvents.map((event) => <EventCard key={event.id} event={event} />)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center"><Clock3 className="mx-auto size-8 text-slate-300" /><p className="mt-3 font-semibold">No encontramos eventos con esos filtros.</p><p className="mt-1 text-sm text-slate-500">Prueba con otra búsqueda o categoría.</p></div>}
      </section>
    </main>
  )
}
